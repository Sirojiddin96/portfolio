const project = [
  {
    id: '1',
    name: '골프존 중고 마켓',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT'],
    image: [require('../../assets/golfzon.png')],
    description:
      '골프존 중고 마켓 is the Platform where we people and the company sell and buy used and new golf tools. This Project has both Website and webapp which works on both in IOS and Android OS',
    tools: ['VS CODE', 'GITHUB'],
    length: '7 Months',
    position: 'FrontEnd',
  },
  {
    id: '2',
    name: 'MeditLink & Medit Projects',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT', 'WEBGL', 'React.JS', 'TYPESCRIPT'],
    image: [
      require('../../assets/meditlink.png'),
      require('../../assets/tag.png'),
      require('../../assets/viewoptions.png'),
    ],
    description:
      'MeditLink is the Platform where clinics and dental labs can manage their patients information. Clinics can make partnership, order cases.',
    tools: ['VS CODE', 'GITHUB Enterprise', 'GitKraken'],
    length: 'Always',
    position: 'FrontEnd',
    link: 'https://www.meditlink.com/'
  },
  {
    id: '3',
    name: 'WellFamilyHouse',
    skills: ['React Native Expo', 'Redux', 'Redux Saga', 'JAVASCRIPT'],
    description:
      'WellFamily house is the mobile app project which is used between family members to communicate and control the health of old people in the Family. App contains: Family section, Health Check-up test, Health Exercises, Memory test game, Creating and communicating with Members, and many more. I would like to describe it in detail in Person',
    tools: ['VS CODE', 'GITHUB'],
    length: '6 Months',
    position: 'FrontEnd',
  },
  {
    id: '4',
    name: 'YodAll',
    skills: ['React Native', 'Redux Toolkit', 'JAVASCRIPT', 'NODEJS', 'EXPRESS', 'MONGODB', 'AWS'],
    description: `YodAll is the  
        Cross Platform mobile app which user can learn new words as dictionary and scheduled and planned way. 
        It is ongoing personal project which was published recently on Android version. 
        You can check app ANDROID version in below link.`,
    tools: ['VS CODE', 'GITHUB', 'GitKraken'],
    length: 'Always',
    position: 'Full-Stack',
    link: 'https://play.google.com/store/apps/details?id=com.yod_all'
  },
  {
    id: '5',
    name: 'Block IP Management System',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT'],
    description: 'Block IP Management System is which manage the block or allow the IP. Project has CRUD operations, DATA TABLE GRIDs, and many more.',
    tools: ['VS CODE', 'GITHUB'],
    length: '5 Months',
    position: 'FrontEnd',
  },
  {
    id: '6',
    name: 'Nebula Broker Management System',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT'],
    description: 'Nebula Broker Management System is the system which gets loads from relay and show in loads lists. Project has Driver lists, Loads List, Tracking loads, and many more.',
    tools: ['VS CODE', 'GITHUB'],
    length: '5 Months',
    position: 'FrontEnd',
  },
  {
    id: '7',
    name: 'SCON Project',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT'],
    description: 'SCON Project is the charity style e-commerce platform where sponsors can sell new or used items which can be lessons or books. Earned money can be used for charity. I created Register Used Item, UsedItem Lists, and many more.',
    tools: ['VS CODE', 'GITHUB'],
    length: '2 months',
    position: 'FrontEnd',
  },
  {
    id: '8',
    name: 'Hanhwa Total Management System',
    skills: ['Vue.JS', 'Vuex', 'JAVASCRIPT'],
    description: 'Hanwha Total Management System is the platform which manage the company and its employees. Project creates Report about Logistics, Sales, and many more.',
    tools: ['VS CODE', 'GITHUB'],
    length: '5 Months',
    position: 'FrontEnd',
  },
];

export default project;
